...
CurrentSpace=$(yabai -m query --spaces --space | jq -re ".")
...
if [[ $CurrentlyVisibleSpaceNames == *$firstSpaceName* ]]; then
    if [ -n ${CurrentlyFocusedWindowID} ]; then # (-n) >> != null
        if [ $CurrentlyFocusedDisplay -ne $inputKeyNumber ]; then
            $(skhd -k "ctrl + cmd - $secondSpacenumber") # shortcut for changing spaces with SIP Enabled
    ...
    if [ -n ${CurrentlyFocusedWindowID} ]; then # (-n) >> != null
        if [ $CurrentlyFocusedDisplay -ne $inputKeyNumber ]; then
            $(skhd -k "ctrl + cmd - $firstspacenumber") # shortcut for changing spaces with SIP Enabled
            focusWindow $CurrentlyFocusedWindowID
        else
